            ...
            if not predicted_returns.empty:
                ...
                with st.expander("Recommended Stocks Based on Predicted Returns"):
                    for stock in recommended_stocks:
                        st.write(f"Stock: {stock[0]}, Predicted Return: {stock[2]}, Beta: {stock[1]['beta']}")
                ...
                def monte_carlo_simulation(predicted_returns, iterations=1000):
                    ...
                    for _ in range(iterations):
                        price_series = [money]
                        for return_rate in predicted_returns:
                            price_series.append(price_series[-1] * (1 + return_rate / 100))
                ...
                # Plot results
                fig = go.Figure()
                fig.add_trace(go.Histogram(x=simulated_prices, nbinsx=50, name='Simulated Prices', opacity=0.75))
